...
**2026-06-26 follow-up — per-instance transport knobs (webhook auto-register + WS disable):** two per-instance config fields (defaults preserve current behaviour), threaded `RawInstanceConfig → WcppConfig` in `config.ts` and gated in `client.ts:connect()/disconnect()`:
- **`webhookRegister`** (default false) — when true, `connect()` re-registers THAT account's WCPPM webhook **Remove-then-Set** (`/Webhook/Remove` then `/Webhook/Set` at `webhookUrl`) and `disconnect()` removes it. This is the feature the multi-instance refactor orphaned: the register/remove code never left `client.ts`, but `resolveInstance` stopped threading `webhookUrl` and gated registration on the (now always-false) per-instance `webhookEnabled`. Registration is **decoupled** from the shared local listener — it only tells WCPPM where to push. `webhookUrl` is **per-instance** (the public reverse-proxied listener URL); the signing secret is the **shared top-level `webhookSecret`** (threaded per-instance) so signatures verify. `/Webhook/*` is **operator-safe** — NOT in the forbidden `/Login|/User|/Admin` set.
- **`wsEnabled`** (default true) — set false to skip the WS push longlink for that account (middleware/WCPPM on different LANs where WS is unreliable); inbound then arrives via webhook only. Only `=== false` disables it.
- **Warn-only guards (`main.ts`, `[cfg]`):** `wsEnabled:false` + `webhookRegister:false` → WARN (inbound depends on an operator-set webhook); `webhookRegister:true` + shared listener off → WARN (pushes 404). The `[cfg]` startup snapshot now prints `ws=on/off webhookReg=on/off`.
